...
public class AttributeEditingOnboarder implements Onboarder {

    private AttributeEditingOnboarder() {
        super();
    }
    ...
    private static final class InstanceHolder {
        private static final AttributeEditingOnboarder INSTANCE = new AttributeEditingOnboarder();
        private InstanceHolder() {
            super();
        }
    }

    public static AttributeEditingOnboarder getInstance() {
        return AttributeEditingOnboarder.InstanceHolder.INSTANCE;
    }
     ...
     */
    private static Logger logger= LogManager.getLogger(AttributeEditingOnboarder.class);
    ...
    public boolean validate(Token token, IAMExtensionV2 iamExtension, UserIciciTO userTO, AccountWE account, User user) throws AuthException {
        logger.log(Level.DEBUG, "<<<<< PERFORMANCE " + "Start ->AttributeEditingOnboarder -> validate |Epoch:"+System.currentTimeMillis());
            ...
            if (enableCrypto) {
              ...
              /*  if (accountWE.getCryptoDID() != null && !accountWE.getCryptoDID().isEmpty()) {
                ...
                for (AttributeTO attribute : accountWE.getAttributes()) {
                    if (attribute.getSignTransactionId() == null || attribute.getSignTransactionId().isEmpty()) {
                        return false;
                    }
                }
                accountWE = account;
                logger.log(Level.DEBUG, "<<<<< PERFORMANCE " + "End ->AttributeEditingOnboarder -> validate |Epoch:"+System.currentTimeMillis());
                return isValid;
            }
            logger.log(Level.DEBUG, "<<<<< PERFORMANCE " + "End ->AttributeEditingOnboarder -> validate |Epoch:"+System.currentTimeMillis());
            return true;
    ...
    public void process(Token token, IAMExtensionV2 iamExtension, UserIciciTO userTO, AccountWE account, User user, Session session) throws AuthException {
        try {
            ...
            accountWE = account;
            accountWE = iamExtension.editAttributesAndSignTransaction(accountWE, accountWE.getId(), token);
            setAccountDetails(accountWE, account);
        } catch (IAMException e) {
            logger.log(Level.ERROR, e.getMessage(), e);
            logger.log(Level.DEBUG, "<<<<< PERFORMANCE " + "End ->AttributeEditingOnboarder -> process | ERROR |Epoch:"+System.currentTimeMillis());
            throw iamExceptionConvertorUtil.convertToAuthException(e);
        }
    }
